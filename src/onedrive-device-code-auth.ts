...
async function main() {
  ...
  console.log("\nSign-in successful.");
  console.log("Access token (short-lived, ignore this):", result.accessToken.slice(0, 20) + "...");
  ...
  if (!firstKey) {
    throw new Error("No refresh token found in cache — check the requested scopes include offline_access.");
  }
